...
async def _edit_menu(query: CallbackQuery, text: str, markup: InlineKeyboardMarkup):
    """Edits query.message in place (caption if it's a photo, text
    otherwise) so navigating the menu doesn't spam new messages."""
    try:
        ...
        logger.warning(f"Failed to edit test-series menu: {e}")
...
async def ts_main(client: Client, query: CallbackQuery):
    ...
    await _edit_menu(query, text, markup)
