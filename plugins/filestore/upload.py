...
def _not_awaiting_settings_reply(_, __, message: Message) -> bool:
    ...
    media) that was actually meant for an in-progress prompt elsewhere —
    most notably /setting's 'start_photo' field, and the Test Series
    admin panel's 'set institute/series image' prompts — where an admin
    sending a picture should set it as that image, not get an
    upload/link-generation prompt.

    The Test Series check imports AWAITING_TS locally (not at module
    level) to avoid a circular import: test_series.py already imports
    BATCH_SESSIONS from this module at load time.
    """
