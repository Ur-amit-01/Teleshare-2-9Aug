...
async def handle_admin_media(client, message: Message):
    ...
    # Held for the append below, so the "finish" button can't pop the
    # session mid-write — otherwise a file sent moments earlier could
    ...
    async with _lock_for(admin_id):
        if admin_id in BATCH_SESSIONS:
            ...
            # One counter per group of files received "in one go" (a single
            # file, or a whole album) — every new group replaces the old
            # counter message (delete + resend) rather than editing it, so
            # the counter always reappears at the bottom of the chat.
